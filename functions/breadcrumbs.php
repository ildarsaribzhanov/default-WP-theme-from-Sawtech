...
function kama_breadcrumbs($sep = ' » ')
{
	...
	if (is_front_page()) {
		return print $open_bc . ($paged ? sprintf($pg_patt,
				get_bloginfo('url')) : '') . $l['home'] . $pg_end . $close_bc;
		...
		$out   = ($paged ? sprintf($pg_patt,
				get_author_posts_url($q_obj->ID, $q_obj->user_nicename)) : '') . sprintf($l['author'],
				$q_obj->display_name) . $pg_end;
		...
		if (is_year()) {
			...
			$out = $y_link . $sep . ($paged ? sprintf($pg_patt, $m_url) : '') . sprintf($l['month'],
					get_the_time('F')) . $pg_end;
		} elseif (is_day()) {
	...
	elseif ($wp_post_types[$post->post_type]->hierarchical) {
		...
		$pg_term_start = ($paged && $term->term_id) ? sprintf($pg_patt,
		...
		if (is_attachment()) {
			if ( ! $post->post_parent) {
				...
				$out = crumbs_tax($term->term_id, $term->taxonomy, $sep, $patt) . sprintf($patt,
						get_permalink($post->post_parent),
						get_the_title($post->post_parent)) . $sep . $post->post_title;
			}
		} elseif (is_single()) {
			$out = crumbs_tax($term->parent, $term->taxonomy, $sep, $patt) . sprintf($patt,
					get_term_link((int)$term->term_id, $term->taxonomy), $term->name) . $sep . $post->post_title;
