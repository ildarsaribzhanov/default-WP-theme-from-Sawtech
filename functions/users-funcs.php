...
function registered_new_user($user_params)
{
	...
	// заполненность полей
	if ($user_params['login'] == '' || $user_params['pass1'] == '' || $user_params['pass2'] == '' || $user_params['email'] == '' || $user_params['phone'] == '' || $user_params['lname'] == '' || $user_params['fname'] == '' || $user_params['company'] == '' || $user_params['good_prav'] == ''
...
{
	// заполненность полей
	if ($param['email'] == '' || $param['phone'] == '' || $param['lname'] == '' || $param['fname'] == ''
	...
	// обновление пароля
	if (isset($param['pass_act']) && $param['pass_act'] != '' && isset($param['pass1']) && $param['pass1'] != '' && isset($param['pass2']) && $param['pass2'] != '') {
...
function update_avatar($file)
{
	...
	if (is_array($ava_url)) {
		...
		if ($filename && strripos($filename, 'http') !== false && strripos($filename,
				'com/') !== false && strripos($filename, 'vk.me/') !== false
		) {
...
function upload_file($file_array)
{
	...
	if ($file_array['size'] > 1000000) {
		array_push($err,
			'<p class="reg_err">Размер файла слишком большой.<br />Допускается загружать аватар до 1 Мб</p>');
	...
	if ( ! in_array($file_format, $formats)) {
		array_push($err,
			'<p class="reg_err">Недопустимый формат файла. Допускается загружать только графические файлы .jpg, .png, .gif.</p>');
...
function get_reset_pass_link($email = '')
{
	...
	$wpdb->update($table_users, array('user_activation_key' => $reset_nonce), array('ID' => $user->ID), array('%s'),
		array('%d'));
...
function is_current_reset_key($email = '', $nonce = '')
{
	...
	$table_users = $wpdb->prefix . 'users';
	$user_id     = $wpdb->get_var($wpdb->prepare("SELECT ID FROM $table_users WHERE user_email = %s AND user_activation_key = %s",
		$email, $nonce));
	...
	$wpdb->update($table_users, array('user_activation_key' => ''), array('ID' => $user->ID), array('%s'), array('%d'));
